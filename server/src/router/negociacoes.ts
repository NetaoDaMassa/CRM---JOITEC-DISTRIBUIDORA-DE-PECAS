...
export const negociacoesRouter = router({
  cobrancasListar: featureProcedure('negociacoes').query(async ({ ctx }) => {
  ...
  cobrancaCriar: featureProcedure('negociacoes')
    .input(z.object({ clienteId: z.number(), canal: z.enum(['whatsapp', 'ligacao', 'email']), retornoCliente: z.string().min(1) }))
